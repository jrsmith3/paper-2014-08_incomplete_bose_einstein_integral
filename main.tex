...
Divide the partial sum of the lower tems of the geometric series by $l^{s}$

\begin{equation} \label{eq:44}
\frac{G_{l}^{lo}(z)}{l^{s}} = \frac{z^{1}}{l^{s}} + \frac{z^{2}}{l^{s}} + \frac{z^{3}}{l^{s}} + ... + \frac{z^{l}}{l^{s}}
\end{equation}

Compare the terms from the two partial sums and note that each term from the polylogarithm series is greater than or equal to the corresponding term of the geometric series.

\begin{tabular}{ccc}
$n$ & $\Li_{s,l}^{lo}(z)$ & $\frac{G_{l}^{lo}(z)}{l^{s}}$ \\
\hline
$1$ & $z$ & $\frac{z}{l^{s}}$ \\
$2$ & $\frac{z^{2}}{2^{s}}$ & $\frac{z^{2}}{l^{s}}$ \\
$3$ & $\frac{z^{3}}{3^{s}}$ & $\frac{z^{3}}{l^{s}}$ \\
... & ... & ... \\
$l$ & $\frac{z^{l}}{l^{s}}$ & $\frac{z^{l}}{l^{s}}$ \\
\end{tabular}

\noindent Thus

\begin{equation} \label{eq:45}
\Li_{s,l}^{lo}(z) > \frac{G_{l}^{lo}(z)}{l^{s}}
\end{equation}

Noting the inequalities of Eqs. \ref{eq:41} and \ref{eq:45}, the uncertainity ratio $U$ is bounded by

\begin{equation} \label{eq:46}
U > \frac{G_{l}^{lo}(z)}{G_{hi}^{lo}(z)} \frac{(l+1)^{s}}{l^{s}}
\end{equation}

\noindent which is to say that the uncertainty ratio of the polylogarithm will be better (higher) than the uncertainty ratio of the partial sum of a geometric series to its remainder. The numerator and denominator of Eq. \ref{eq:46} can be replaced and the expression simplified to

\begin{equation} \label{eq:47}
U > U_{G} = z^{-l} - 1
\end{equation}

\noindent where $U_{G}$ is the uncertainty ratio of the geometric series. Therefore, an arbitrary precision $U_{G}$ can be selected and the number of terms $l$ of the polylogarithm partial sum to achieve that precision can be determined for a given value of $z$

\begin{equation} \label{eq:48}
l = -\frac{\log(U_{G} + 1)}{\log(z)}
\end{equation}
